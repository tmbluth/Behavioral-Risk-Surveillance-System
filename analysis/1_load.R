...
train_11 <- fread('data/2011.csv', 
   select = c('_STATE', 'GENHLTH',  'PHYSHLTH', 'MENTHLTH',  'HLTHPLN1',
...
train_12 <- fread('data/2012.csv',
  ...
  mutate(year = '2012', RACE = as.factor(`_IMPRACE`)) %>% 
...
train_14 <- fread('data/2014.csv',          
   select = c('_STATE', 'GENHLTH',  'PHYSHLTH', 'MENTHLTH',  'HLTHPLN1',
              ...
              '_SMOKER3', '_DRNKMO4', '_RFBING5', 'PDIABTST', 'PREDIAB1',
...
all_years$DRNKMON <- ifelse(all_years$DRNKMON < 0.03, 0, all_years$DRNKMON) # Anyone that drinks less than once a month are considered a 0
NA_prop(all_years)
